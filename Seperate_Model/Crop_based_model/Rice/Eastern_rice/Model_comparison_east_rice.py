...
def preprocessing():
    data = pd.read_csv('Eastern_rice.csv')
    data = data.dropna()
...
def comparison(Elastic_data,RF_data, XGB_data, SVR_data):
    ...
    print(comparison_df)
    comparison_df.to_csv('Eastern_model_comparison.csv', index=False)
...
comparison_df = comparison(Elastic_data,RF_data, XGB_data, SVR_data)